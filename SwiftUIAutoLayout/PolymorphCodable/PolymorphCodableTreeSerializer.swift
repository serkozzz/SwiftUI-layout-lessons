...
class PolymorphCodableTreeSerializer {
    ...
    func decodeTree(data: Data) throws -> PCNode {
    ...
    func decodeNode(dict: [String: Any]) throws -> PCNode {
        // 1) kind
        ...
        switch kind {
        case .base:
            node = try decoder.decode(PCNode.self, from: bodyData)
        case .variable:
            node = try decoder.decode(PCCVariableNode.self, from: bodyData)
        case .view:
            node = try decoder.decode(PCViewNode.self, from: bodyData)
        }
